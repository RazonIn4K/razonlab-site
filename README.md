...
This repository preserves the standalone Razon Live Lab source while useful material is evaluated for migration. The accepted portfolio role is Razon Lab, the experimental research and prototyping arm of RazonWorks.
...
learning intent without settling legal identity. It also replaces the outdated
...
- **Not owned here:** commercial service intake, curriculum, or David's personal portfolio and contact hub.

RazonWorks owns the commercial content role, High Encode Learning owns the
learning content role, DavidTiz owns personal portfolio and collaboration
context, and Razon Lab owns experiment records as the experimental arm of
RazonWorks. Those content roles do not identify a legal entity, DBA, contracting
party, merchant or payment recipient, privacy data controller, domain owner, or
the legal relationship between RazonWorks and High Encode Learning.

The public content role is governed by the accepted RazonWorks ADR, charter, and
ledger. The unresolved identity decision is tracked separately in
`RazonIn4K/razonworks#160`. Read
[docs/BRAND-BOUNDARY.md](docs/BRAND-BOUNDARY.md) for the local evidence and
migration rules.
...
- RazonWorks: `https://razonworks.com/request` (current commercial request route; contracting and payment roles unresolved)
- High Encode Learning: `https://highencodelearning.com/` (current learning surface; no legal or entity relationship to RazonWorks asserted)

## Candidate migration state

RazonWorks `/lab` and `/es/lab` are the accepted migration targets and exist in
local source as of 2026-08-10. This repository has not verified a preview or
production deployment. Do not describe the routes as live or treat their
content role as proof of a legal, payment, or contracting relationship.

No standalone-domain redirect is authorized until both target routes are
hosted and verified, useful source material is preserved, the identity decision
is resolved where it affects publication, and the user approves the cutover.
