...
Phase 0 is documentation only. It does not authorize or complete any of the following:
...
The RazonWorks `/lab` and `/es/lab` routes are planned source destinations. Phase 0 does not implement, deploy, or host them. No redirect from the standalone domain is authorized until both routes exist, their hosted behavior is verified, useful source material has been preserved, and the user approves the cutover.

## Safe migration order

1. Inventory the current source and classify each useful item as preserve, rewrite, summarize, move, or retire.
...
For documentation-only changes, review the diff and run:
